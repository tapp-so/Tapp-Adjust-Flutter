...
class ComTappSoAdjust {
  ...
  /// Fire a tapp-event with an enum or custom string, plus optional metadata.
  Future<void> handleTappEvent({
    ...
    debugPrint(
        '[ComTappSoAdjust.handleTappEvent] actionName=$actionName customValue=$customValue metadata=$metadata');
    if (!Platform.isAndroid && !Platform.isIOS) {
  ...
  Map<String, Object>? _normalizeMetadata(Map<String, Object?>? metadata) {
    ...
    metadata.forEach((key, value) {
      ...
      final isSupported = value is String || value is bool || value is num;
      if (!isSupported) {
        throw ArgumentError(
          'metadata["$key"] has unsupported type ${value.runtimeType}. '
          'Allowed types: String, num, bool.',
        );
      }
